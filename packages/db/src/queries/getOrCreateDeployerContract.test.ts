...
import { getOrCreateDeployerContract } from './getOrCreateDeployerContract.js';
...
describe('getOrCreateDeployerContract', () => {
  ...
  it('should throw if encountered unexpected error', async () => {
    const badNewDeployer = {
      address: null as unknown as `0x${string}` // TODO: ideally we should mock the DB error instead of doing this
    };
    await expect(
      getOrCreateDeployerContract(db, badNewDeployer)
    ).rejects.toThrow();
    const dbContracts = await db.select().from(contracts);
