...
CREATE TABLE "glider_portfolio_rebalances_log" (
	...
	"log_label" smallint NOT NULL,
	"created_at" timestamp DEFAULT now() NOT NULL,
	"glider_rebalance_id" text,
	"response" jsonb
);
--> statement-breakpoint
CREATE TABLE "glider_portfolio_rebalances_log_labels" (
...
ALTER TABLE "glider_portfolio_rebalances_log" ADD CONSTRAINT "glider_portfolio_rebalances_log_log_label_glider_portfolio_rebalances_log_labels_id_fk" FOREIGN KEY ("log_label") REFERENCES "public"."glider_portfolio_rebalances_log_labels"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
